...
(* simplifie : int -> int list list -> int list list 
   ...
   let simplifie i clauses =
    filter_map (fun x -> if List.mem i x then None else Some(filter_map (fun j -> if j <> -i then Some(j) else None) x)) (clauses)
    ...
    let rec unitaire clauses =
      match clauses with
        |t::q -> if List.length t=1 
          then List.hd t else unitaire q
...
let removeElt i clauses = 
  filter_map (fun x -> Some(filter_map (fun j -> if j <> i && j<> -i then Some(j) else None) x)) clauses
let rec pur clauses =
  let maliste = List.flatten clauses in 
  if List.exists (fun w -> w = -(List.hd maliste)) maliste
  then pur (removeElt (List.hd maliste) clauses) else List.hd maliste
...
let rec solveur_dpll_rec clauses interpretation=
  ...
  with | _ -> 
    try solveur_dpll_rec (simplifie (pur clauses) (clauses)) ((pur clauses)::interpretation)
